...
impl<'a> Invoker<'a> {
    pub fn name(&self) -> String {
        format!("{}{}", INVOKER_PREFIX, self.func.ident)
    }
...
impl<'a> CommonInvokerTokens<'a> {
    ...
    fn iter_args(&self) -> impl Iterator<Item = (&'a Ident, &'a Type)> {
        self.func.decl.inputs.iter().map(|x| match x {
            FnArg::Captured(arg) => (
                match &arg.pat {
...
impl ToTokens for Invoker<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let ident = Ident::new(
            &format!("{}{}", INVOKER_PREFIX, self.func.ident.to_string()),
            self.func.ident.span(),
        );
        ...
        if self.is_orchestration {
            ...
            .to_tokens(tokens);
        } else if self.func.asyncness.is_some() {
            quote!(
                #[allow(dead_code)]
                fn #ident(
                    __req: ::azure_functions::rpc::InvocationRequest,

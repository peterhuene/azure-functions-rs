...
#[binding(name = "serviceBus", direction = "out", validate = "validate")]
    ...
    fn validate(&self) -> Result<(), String> {
        if self.queue_name.is_some() {
            if self.topic_name.is_some() || self.subscription_name.is_some() {
                return Err("service bus binding cannot have both `queue_name` and either `topic_name` or `subscription_name` specified".to_owned());
            }
        } else if self.topic_name.is_none() || self.subscription_name.is_none() {
            return Err("service bus binding must have either `queue_name` or both `topic_name` and `subscription_name` specified".to_owned());
        }
        Ok(())
    }
}
...
mod tests {
    ...
    fn it_serializes_to_json() {
        ...
        assert_eq!(
            ...
            r#"{"type":"serviceBus","direction":"out","name":"foo","queueName":"bar","topicName":"baz","subscriptionName":"jam","connection":"cake"}"#
        );
    }
    ...
    fn it_requires_the_name_attribute_argument() {
        should_panic(
            || {
                let _: ServiceBus = (vec![], Span::call_site()).into();
            },
    ...
    fn it_requires_queue_name_or_topic_subscription() {
        should_panic(
            ...
            "service bus binding must have either `queue_name` or both `topic_name` and `subscription_name` specified",
        );
        ...
        should_panic(
            ...
            "service bus binding cannot have both `queue_name` and either `topic_name` or `subscription_name` specified",
        );
    ...
    fn it_converts_to_tokens() {
        ...
        assert_eq!(tokens, r#"::azure_functions::codegen::bindings::ServiceBus{name:::std::borrow::Cow::Borrowed("foo"),queue_name:Some(::std::borrow::Cow::Borrowed("bar")),topic_name:Some(::std::borrow::Cow::Borrowed("baz")),subscription_name:Some(::std::borrow::Cow::Borrowed("jam")),connection:Some(::std::borrow::Cow::Borrowed("cake")),}"#);
    }
}

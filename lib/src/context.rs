...
impl<'a> Context<'a> {
    ...
    pub fn current_function_directory(&self) -> Option<PathBuf> {
        self.function_app_directory().map(|p| p.join(self.name))
    }

    /// Gets the directory for the current Azure Function Application.
    pub fn function_app_directory(&self) -> Option<PathBuf> {
        env::current_exe()
